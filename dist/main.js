...
fileInput.addEventListener("click", () => {
    ...
    audioAnalyser = audioAnalyser !== null && audioAnalyser !== void 0 ? audioAnalyser : audioCtx.createAnalyser();
    if (audioSource)
        audioSource.stop();
...
window.requestAnimationFrame(function draw() {
    if (audioAnalyser) {
        ...
        freqData = freqData !== null && freqData !== void 0 ? freqData : new Float32Array(frequencyBinCount);
        positions = positions !== null && positions !== void 0 ? positions : new Float32Array(freqData.length * 4);
        audioAnalyser.getFloatFrequencyData(freqData);
        for (let i = 0; i < freqData.length; i++) {
            let j = i * 4;
            let x = i / freqData.length;
            let normalized = (freqData[i] - minDecibels) / (maxDecibels - minDecibels);
            positions[j] = x;
